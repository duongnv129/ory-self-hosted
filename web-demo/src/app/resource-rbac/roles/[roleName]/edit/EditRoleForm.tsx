...
export function EditRoleForm({ roleName }: EditRoleFormProps) {
  ...
  const { role: currentRole, updateRole, isLoading: roleLoading, isError: roleError } = useRole(roleName); // Use useRole for role operations and data
  ...
  useEffect(() => {
    if (currentRole && !isInitialized) {
      console.log('Initializing form with role:', currentRole); // Debug log
      setFormData({
  ...
  }, [roleName, roleLoading, currentRole, roleError, isInitialized]);
